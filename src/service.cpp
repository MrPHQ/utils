...
namespace UTILS {namespace SERVICE {
	...
	int Service::AnalyseCmd(int argc, char* argv[]) {
	...
	int Service::AnalyseCmdEx(const TCHAR* ptszCmdLine, PSERVICE_CMD pCmd){
	...
	//更新服务状态
	BOOL Service::UpdateServiceStatus(SERVICE_STATUS_HANDLE hStatus, DWORD dwStatus, DWORD dwErrCode, DWORD dwWaitHint)
	{
		if (NULL == hStatus){
			return FALSE;
		}
	...
	//注册服务
	BOOL Service::RegisterService(LPCTSTR pServiceName, LPTSTR pServiceDisplayName)
	{
	...
	//注销服务
	BOOL Service::UnregisterService(LPCTSTR pServiceName)
	{
		BOOL br = FALSE;
		SC_HANDLE hSCManager = NULL;
		SC_HANDLE hSCService = NULL;
	...
	//开始服务
	BOOL Service::StartupService(LPCTSTR pServiceName)
	{
		SC_HANDLE hSCManager = NULL;
		SC_HANDLE hSCService = NULL;
		BOOL br = FALSE;
	...
	//停止服务
	BOOL Service::StopService(LPCTSTR pServiceName)
	{
		SC_HANDLE hSCManager = NULL;
		SC_HANDLE hSCService = NULL;
		BOOL br = FALSE;
	...
	//重置服务
	BOOL Service::ReconfigureService(LPCTSTR pServiceName, LPTSTR pDesc)
	{
		SC_HANDLE hSCManager = NULL;
		SC_HANDLE hSCService = NULL;
		BOOL br = FALSE;
	...
	//判断服务是否运行
	BOOL Service::IsServiceRunning(LPCTSTR pServiceName)
	{
		SC_HANDLE hSCManager = NULL;
		SC_HANDLE hSCService = NULL;
		BOOL br = FALSE;
	...
	//主服务控制函数
	void WINAPI Service::ServiceHandler(DWORD dwControl)
	{
	...
	//主服务主函数
	void WINAPI Service::ServiceMain(DWORD dwArgc, LPTSTR *pArgv)
	{
	...
	//看管服务控制函数
	void WINAPI Service::WatchDogServiceHandler(DWORD dwControl)
	{
	...
	//看管服务主函数
	void WINAPI Service::WatchDogServiceMain(DWORD dwArgc, LPTSTR *pArgv)
	{
	...
	//服务运行状态检测主函数
	DWORD WINAPI Service::TimeCheckProcLoop(LPVOID pParam)
	{
		DWORD dwWait = 0;
