...
namespace UTILS {namespace API {
	...
	* @return
	*/
	UTILS_API std::string ConvertIp(unsigned int nIP)
	{
		std::string ip;
		char szTmp[32];
		// 直接右移24位
		_snprintf_s(szTmp, _TRUNCATE, "%d", (nIP >> 24));
		ip.append(szTmp);
		ip.append(".");
		// 将高8位置0，然后右移16位
		_snprintf_s(szTmp, _TRUNCATE, "%d", (nIP & 0x00FFFFFF) >> 16);
		ip.append(szTmp);
		ip.append(".");
		// 将高16位置0，然后右移8位
		_snprintf_s(szTmp, _TRUNCATE, "%d", (nIP & 0x0000FFFF) >> 8);
		ip.append(szTmp);
		ip.append(".");
		// 将高24位置0
		_snprintf_s(szTmp, _TRUNCATE, "%d", (nIP & 0x000000FF));
		ip.append(szTmp);
		return ip;
	}
}}